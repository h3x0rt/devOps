...
// Получаем фильтры
$attrQuery = "
    SELECT DISTINCT a.name AS attribute_name, pa.value 
...
";
$attrStmt = $conn->prepare($attrQuery);
$attrStmt->bind_param('i', $categoryId);
$attrStmt->execute();
$attrResult = $attrStmt->get_result();

$filters = [];
while ($row = $attrResult->fetch_assoc()) {
    $filters[$row['attribute_name']][] = $row['value'];
}
$attrStmt->close();
...
foreach ($_GET as $name => $value) {
    if (!empty($value)) {
        $whereClauses[] = "(a.name = ? AND pa.value = ?)";
        $types .= 'ss';
        $params[] = $name;
        $params[] = $value;
    }
...
if (!empty($whereClauses)) {
    $filterSql = "
        ...
        WHERE p.category_id = ? AND " . implode(" AND ", $whereClauses);
...
if (!empty($filteredProductIds)) {
    $placeholders = implode(',', array_fill(0, count($filteredProductIds), '?'));
    $types = str_repeat('i', count($filteredProductIds));
    $stmt = $conn->prepare("SELECT * FROM products WHERE ID IN ($placeholders)");
    $stmt->bind_param($types, ...$filteredProductIds);
} else {
...
// Получение атрибутов по ID товаров
$attributesData = [];
if (!empty($productIds)) {
    ...
    $attrSql = "
        SELECT pa.product_id, a.name AS attr_name, pa.value
        FROM productattributes pa
        JOIN attributes a ON a.ID = pa.attribute_id
    ...
    $attrStmt->bind_param($types, ...$productIds);
    $attrStmt->execute();
    $attrResult = $attrStmt->get_result();

    while ($row = $attrResult->fetch_assoc()) {
        $attributesData[$row['product_id']][] = [
            'name' => $row['attr_name'],
            'value' => $row['value']
        ];
    }
...
// Объединяем товары с атрибутами
foreach ($productData as $id => $product) {
    $product['attributes'] = $attributesData[$id] ?? [];
...
<body>
    ...
    <nav>
        <div class="content nav-content">
            ...
            <a href="profnastil.php"><div class="nav-button nav-active">Профнастил</div></a>
    ...
    <div class="content">
        ...
        <form method="get" class="filter">
            <div class="form-filter">
                <?php foreach ($filters as $attrName => $values): ?>
                <dl>
                    <dt><label><?= htmlspecialchars($attrName) ?>: </label></dt>
                    <dd><select name="<?= htmlspecialchars($attrName) ?>">
                    <option value="">Все</option>
                    <?php foreach (array_unique($values) as $val): ?>
                        <option value="<?= htmlspecialchars($val) ?>" <?= (isset($_GET[$attrName]) && $_GET[$attrName] === $val) ? 'selected' : '' ?>>
...
<!-- Вывод товаров -->
        <?php if (isset($products[$categoryName]) && count($products[$categoryName]) > 0): ?>
            <?php foreach ($products[$categoryName] as $product): ?>
                <div class="card">
                    ...
                    <div class="incard">
                        ...
                        <?php endforeach; ?>
                        <button class="add-to-cart-btn" data-product-id="<?= htmlspecialchars($product['ID']) ?>" data-quantity="1" title="Добавить в корзину">
                            Добавить в корзину
                        </button>
                    </div>
                </div>
