...
<body id="dialog">
    ...
    <nav>
        <div class="content nav-content">
            <a href="proftruba.php"><div class="nav-button nav-active">Профильные трубы</div> </a>
    ...
    <div class="content">
        <span class="description">Профильные трубы – полые и, соответственно легче круглых. 
            Трубы такого типа можно использовать при строительстве зданий любой формы и назначения.
            Они также очень прочные и способны выдерживать большие нагрузки, не деформируясь при этом.
            Очень устойчивы к внешнем воздействиям и агрессивным погодным условиям.
        </span>
        <ul>
            <li><a href="#proftruba">Профильные трубы</a></li>
            <li><a href="#krug">Круглые трубы</a></li>
        </ul>
        <h2 align="center" id="proftruba">Профильные трубы</h2>
        <div class="catalog">
            <?php if (isset($products['Профильная труба'])): ?>
                <?php foreach ($products['Профильная труба'] as $product): ?>
                    <div class="card">
                        ...
                        <div class="incard">
                            <dl><dt>Цена:</dt><dd class="price"><?= htmlspecialchars($product['price']) ?> ₽/<?= htmlspecialchars($product['price_for']) ?></dd></dl>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p>Нет товаров в категории "Профильные трубы".</p>
            <?php endif; ?>
        </div>
        <h2 align="center" id="krug">Круглые трубы</h2>
        <div class="catalog">
        <?php if (isset($products['Круглая труба'])): ?>
            <?php foreach ($products['Круглая труба'] as $product): ?>
                <div class="card">
                    <img src="<?= htmlspecialchars($product['img']) ?>" alt="<?= htmlspecialchars($product['name']) ?>">
                    <div class="product-name">
                        <b><?= htmlspecialchars($product['name']) ?></b>
                    ...
                    <div class="incard">
                        ...
                        <?php foreach ($product['attributes'] as $attr): ?>
                            <dl class="attributes"><dt><?= htmlspecialchars($attr['name']) ?>:</dt><dd><?= htmlspecialchars($attr['value']) ?></dd></dl>
                        <?php endforeach; ?>
                        <button title="Добавить в корзину" class="add-to-cart-btn"  data-quantity="6" data-product-id="<?= htmlspecialchars($product['id']) ?>">
                            Добавить в корзину
                        </button>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <p>Нет товаров в категории "Круглые трубы".</p>
        <?php endif; ?>
        </div>
    </div>
    <footer>
