<?php
session_start();
if(!($_SESSION['role'] === 'admin')){
    echo"<script>alert('У вас недостаточно прав, для входа. Если это ошибка, обратитесь к системному администратору')</script>";
    header("Location: index.php");
}
// Подключение к базе данных
$host = "localhost";
$user = "root";
$password = "";
$dbname = "diplom";
$conn = new mysqli($host, $user, $password, $dbname);

if ($conn->connect_error) {
    die("Ошибка подключения: " . $conn->connect_error);
}

// Получение списка продуктов
$allowedTables = ['products', 'categories','orders', 'orderitems', 'cartitems', 'attributes', 'productattributes', 'userlogs'];

// Получение выбранной таблицы
$selectedTable = $_POST['table'] ?? '';
$data = [];

if (in_array($selectedTable, $allowedTables)) {
    $query = "SELECT * FROM `$selectedTable`";
    $result = $conn->query($query);
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $data[] = $row;
        }
    }
}
else{
    $query = "SELECT * FROM `products`";
    $result = $conn->query($query);
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $data[] = $row;
        }
    }
}

$orders = $conn->query("select * from orders");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <title>Панель администратора</title>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <style>
.responsive-table {
    overflow-x: auto;
    max-width: 100%;
}

.responsive-table table {
    
    border-collapse: collapse;
    
}

.responsive-table th, .responsive-table td {
    padding: 10px;
    text-align: left;
    border: 1px solid #ccc;
}

/* Для мобильных — можно показать таблицу в виде блоков */
@media (max-width: 768px) {
    .responsive-table table,
    .responsive-table thead,
    .responsive-table tbody,
    .responsive-table th,
    .responsive-table td,
    .responsive-table tr {
        display: block;
    }

    .responsive-table thead {
        display: none;
    }

    .responsive-table tr {
        margin-bottom: 15px;
        border: 1px solid #ddd;
        padding: 10px;
    }

    .responsive-table td {
        text-align: right;
        padding-left: 50%;
        position: relative;
    }

    .responsive-table td::before {
        content: attr(data-label);
        position: absolute;
        left: 10px;
        width: 45%;
        white-space: nowrap;
        text-align: left;
        font-weight: bold;
    }
}
</style>
 
</head>
<body>
    <button id="scrollToTopBtn" title="Наверх"><i class="fa-solid fa-arrow-up"></i></button>
    <div id="logoutDialog" class="darkened-background">
        <form id="logoutAlert" class="logoutAlert">
            <h2>Подтвердите выход</h2>
            Вы уверены, что хотите выйти с аккаунтa?
            <button type="submit" class="auth-buttons show">ВЫХОД</button>
        </form>
    </div>
    <div id="editDialog" class="darkened-background">
        <form id="editForm" class="modal-form">
            <span onclick="closeEdit()" class="close">&times;</span>
            <input type="hidden" name="table" id="edit-table">
            <input type="hidden" name="id" id="edit-id">
            <div>
                <h3>Изменение записи</h3>
            </div>
            <div id="edit-fields"></div>
            <button type="submit">Сохранить</button>
        </form>
    </div>
    <div id="auth-dialog" class="darkened-background">
        <form id="authorise" class="authorise" bindsubmit="">
            <h2 style="margin-top: 10px;">АВТОРИЗАЦИЯ</h2>
            <div class="input-wrapper">
                <i class="fas fa-user icon"></i>
                <input id="login-auth" class="inputs" type="text" required placeholder="Логин">
            </div>
            <div class="input-wrapper">
                <i class="fas fa-lock icon"></i>
                <input id="password-auth" class="inputs" type="password" required placeholder="Ваш пароль">
            </div>
            <div id="auth-error-message" class="auth-message"><span></span></div>
            <div class="modal-buttons">
                <button type="submit">Войти</button>
                <a onclick="closeDialogAuth(); openDialogReg()">Нет аккаунта? Регистрация</a>
            </div>
        </form>
    </div>
    <div id="reg-dialog" class="darkened-background">
        <form id="register" class="authorise" bindsubmit="">
            <h2 style="margin-top:10px;">РЕГИСТРАЦИЯ</h2>
            
            <div class="input-wrapper">
                <i class="fas fa-user icon"></i>
                <input id="login-reg" class="inputs" required type="text" placeholder="Введите логин">
            </div>
            <div class="input-wrapper">
                <i class="fas fa-lock icon"></i>
                <input id="password-reg" class="inputs" required type="password" placeholder="Введите пароль">
            </div>
            <div class="input-wrapper">
                <i class="fas fa-lock icon"></i>
                <input id="password-verify-reg" class="inputs" required type="password" placeholder="Подтвердите пароль">
            </div>
            <div id="register-error-message" class="reg-message"><span></span></div>
            <div class="modal-buttons">
                <button type="submit">Зарегистрироваться</button>
                <a onclick="closeDialogReg(); openDialogAuth()">Уже есть аккаунт? Войти</a>
            </div>
        </form>
    </div>
    <header id="main-content">
        <div class="content">
                <a href="index.php">
                    <div class="logo">
                        <span class="ug"><span style="color: rgb(255, 56, 56);">Ю</span>г</span>
                        <span class="belora"><span style="color: rgb(255, 56, 56);">Б</span>елора</span>
                        <span class="metall"><span style="color: rgb(255, 56, 56);">М</span>еталл</span>
                    </div>
                </a>
                <div class="mobile-menu-toggle">
                    <span></span>
                    <span></span>
                    <span></span>
                </div>
            <div class="main-nav">
                <div class="search-nav">
                    <div class="search-container">
                        <form action="search.php" method="GET">
                            <input type="text" required name="search" placeholder="Профнастил C-8" id="search">
                            <button class="search-btn" type="submit">
                                <svg class="search-icon" viewBox="0 0 24 24" width="20" height="20">
                                <path fill="currentColor" d="M15.5 14h-.79l-.28-.27a6.5 6.5 0 0 0 1.48-5.34c-.47-2.78-2.79-5-5.59-5.34a6.505 6.505 0 0 0-7.27 7.27c.34 2.8 2.56 5.12 5.34 5.59a6.5 6.5 0 0 0 5.34-1.48l.27.28v.79l4.25 4.25c.41.41 1.08.41 1.49 0 .41-.41.41-1.08 0-1.49L15.5 14zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
                                </svg>
                            </button>
                        </form>
                    </div>
                    <div class="auth-buttons" id="authorized">
                        <div class="user-info">
                            <i class="fas fa-user"></i>
                            <span id="username" class="username"><?php echo $_SESSION['user_name']?></span>
                        </div>
                        <a href="cart.php"><div class="cart">
                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                xmlns="http://www.w3.org/2000/svg" class="cart-icon">
                            <circle cx="9" cy="21" r="1" />
                            <circle cx="20" cy="21" r="1" />
                            <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61l1.6-8.39H6" />
                            </svg>
                        </div></a>
                        <button onclick="logoutDialog()" class="auth-btn show">ВЫХОД</button>

                        
                    </div>
                    
                    <div class="auth-buttons" id="auth-buttons">
                        <button onclick="openDialogAuth()" id="auth-btn" class="auth-btn show">Вход</button>
                        <button onclick="openDialogReg()" id="reg-btn" class="reg-btn show">Регистрация</button>
                    </div>
                </div>
                <div class="header-info">
                    <span>+7 918 999-99-86<br>Для заказа по звонку</span>
                    
                </div>
            </div>

            
            
        </div>
    </header>
    <aside class="sidebar">
        <nav class="sidebar-nav">
            <div id="workStatus" class="open-message">
            </div>
            
            <a href="proftruba.php" class="sidebar-btn">
                <span class="btn-text">Профильные трубы</span>
            </a>
            <a href="profnastil.php" class="sidebar-btn active">
                <span class="btn-text">Профнастил</span>
            </a>
            <a href="armatura.php" class="sidebar-btn">
                <span class="btn-text">Арматура</span>
            </a>
            <a href="lattice.php" class="sidebar-btn">
                <span class="btn-text">Сетка</span>
            </a>
            <a href="services.php" class="sidebar-btn">
                <span class="btn-text">Наши услуги</span>
            </a>
            <a href="contacts.php" class="sidebar-btn">
                <span class="btn-text">Контакты</span>
            </a>
            <a href="prochee.php" class="sidebar-btn">
                <span class="btn-text">Прочее</span>
            </a>
            <a href="clamp.php" class="sidebar-btn">
                <span class="btn-text">Хомуты</span>
            </a>
        </nav>
    </aside>
    <nav>
        <div class="content nav-content">
            <a href="proftruba.php"><div class="nav-button">Профильные трубы</div> </a>
            <a href="profnastil.php"><div class="nav-button">Профнастил</div></a>
            <a href="armatura.php"><div class="nav-button">Арматура</div></a>
            <a href="services.php"><div class="nav-button">Наши услуги</div></a>
            
                <div id="prochee">
                    <a class="nav-button" style="border: 0px; box-shadow: initial;"  href="prochee.php">Прочее</a>
                    <div class="showMore">
                        <div class="contentMore">
                            <dl><dt><a href="wire.php"><span>Проволока</span></a></dt><dd><a href="insulation.php"><span> Утеплитель</span></a></dd></dl>
                            <dl><dt><a href="metalist.php"><span>Горячекатаный лист</span></a></dt><dd><a href="screws.php"><span> Саморезы/Гвозди</span></a></dd></dl>
                            <dl><dt><a href="osb.php"><span>OSB</span></a></dt><dd><a href="kraska.php"><span> Краска</span></a></dd></dl>
                            <dl><dt><a href="ugolok.php"><span>Уголок метал.</span></a></dt><dd><a href="lattice.php"><span> Сетка</span></a></dd></dl>
                            <dl><dt><a href="kist.php"><span>Кисточки/Валики</span></a></dt><dd><a href="clamp.php">Хомуты</a></dd></dl>
                            <dl><dt style="width: 100%;"><a href="hydroparo.php"><span>Гидроизоляция и Пароизоляция</span></a></dt></dl>
                            <dl><dt style="width: 100%;"><a href="varia.php"><span>Разное</span> </a></dt></dl>
                            <dl><dt style="width: 100%;"><a href="shim.php"><span>Шпильки/Шайбы/Гайки</span> </a></dt></dl>
                        </div>
                    </div>
                </div>
            
            <a href="contacts.php"><div class="nav-button">Контакты</div></a>
        </div>
    </nav>
    <div class="content ">
        <h1 align="center">Панель администратора</h1>
        <ul class="admin-ul">
            
        </ul>
        <h2>Текущие заказы</h2>
        <div class="current_clients">
            <?php
                $result = $conn->query("SELECT * FROM orders ORDER BY created_at DESC");

                if ($result && $result->num_rows > 0): ?>
                <div class="responsive-table">
                    <table border="1">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Телефон</th>
                                <th>Дата создания</th>
                                <th>Статус заказа</th>
                                <th>Примечания</th>
                                <th>Адрес</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    <td data-label="ID"><?= $row['ID'] ?></td>
                                    <td data-label="Телефон"><?= htmlspecialchars($row['phone']) ?></td>
                                    <td data-label="Дата создания"><?= $row['created_at'] ?></td>
                                    <td data-label="Статус заказа"><?= htmlspecialchars(trim($row['status']) !== '' ? $row['status'] : '—') ?></td>
                                    <td data-label="Примечания"><?= htmlspecialchars(trim($row['remark']) !== '' ? $row['remark'] :  '—') ?></td>
                                    <td data-label="Адрес"><?= htmlspecialchars(trim($row['adress']) !== '' ? $row['adress'] : '—') ?></td>
                                    <td><a href="order_details.php?order_id=<?= $row['ID'] ?>" class="btn">О заказе</a></td>
                                    
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
                    </table>
                <?php else: ?>
                    <p>Заявок нет на данный момент</p>
            <?php endif; ?>
                </div>
        </div>
        
        <h2 id="obzor">Обзор таблиц</h2>
        <form method="POST" class="form-row view">
            <label for="table">Выберите таблицу:</label>
            <select name="table" id="table">
                <?php foreach ($allowedTables as $table): ?>
                    <option value="<?= $table ?>" <?= $selectedTable === $table ? 'selected' : '' ?>>
                        <?= $table ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <button type="submit" class="select-table-btn">Применить</button>
        </form>
        <?php if ($selectedTable): ?>
        <h3 style="text-align:center">Таблица: <?= htmlspecialchars($selectedTable) ?></h3>
        <div class="responsive-table">
            <table border="1">
                <?php if (!empty($data)): ?>
                    <thead>
                        <tr>
                            <?php foreach (array_keys($data[0]) as $col): ?>
                                <th><?= htmlspecialchars($col) ?></th>
                            <?php endforeach; ?>
                            <th>Действия</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($data as $row): ?>
                            <tr>
                                <?php foreach ($row as $colName => $cell): ?>
                                    <td data-label="<?= htmlspecialchars($colName) ?>"><?= htmlspecialchars(trim($cell) !== '' ? $cell : '—') ?></td>
                                <?php endforeach; ?>
                                <td data-label="Действия">
                                    <form style="display:inline;" >
                                        <input type="hidden" name="table" value="<?= htmlspecialchars($selectedTable) ?>">
                                        <input type="hidden" name="id" value="<?= reset($row) ?>">
                                        <button type="submit" class="action-btn delete-btn"><i class="fa-solid fa-trash fa-2x"></i></button>
                                    </form>
                                    
                                    <div style="display:inline;">
                                        <input type="hidden" name="table" value="<?= htmlspecialchars($selectedTable) ?>">
                                        <input type="hidden" name="id" value="<?= reset($row) ?>">
                                        <button data-id="<?= $row['ID'] ?>" data-table="<?= $selectedTable ?>" class="action-btn edit-btn"><i class="fa-solid fa-pen-to-square fa-2x"></i></i></button>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                <?php else: ?>
                    <tr><td colspan="100%">Нет данных в таблице</td></tr>
                <?php endif; ?>
            </table>
            <?php endif; ?>
        </div>
        <h2 align="center">Добавление записи</h2>
        <form action="php/add_record.php" id="addRecordForm" method="POST" enctype="multipart/form-data">
            <div class="form-row">
                <label for="table">Выберите таблицу:</label>
                <select name="select-table" id="select-table" required>
                    <option value="">-- Выберите --</option>
                    <option value="products">Товары</option>
                    <option value="categories">Категории</option>
                    <option value="attributes">Атрибуты</option>
                    <option value="productattributes">Атрибуты товаров</option>
                    <option value="orderitems">Товары заказов</option>
                </select>
            </div>

            <div id="formFields">
                
            </div>
            <?php
            if (!empty($_SESSION['error'])) {
                echo '<div class="error-message" style="color: red; font-weight: bold;">' . $_SESSION['error'] . '</div>';
                unset($_SESSION['error']); // очистим после показа
            }
            ?>
            <button class="select-table-btn" type="submit">Добавить</button>
        </form>
    </div>
    <footer>
        <div class="foot-block">

            <div class="foot">
                <a href="tel:+79189999986"><span class="phone">+7 918 999 99 86</span></a><br>
                <a href="tel:+79180000204"><span class="phone">+7 918 0000 204</span></a><br>
                <a href="tel:+79186666023"><span class="phone">+7 988 6666 023</span></a><br>

                <span>Электронная почта: </span><br>
                <span> xanikyan90@mail.ru</span><br>
                <span>Социальные сети:</span><br>
                <div style="display: flex; flex-wrap: wrap;">
                    <div class="social-links">
                        <a href="https://www.instagram.com/@khanikyan_zhanna" target="_blank" title="Instagram">
                            <i class="fab fa-instagram"></i>
                        </a>
                        <a href="https://vk.com/ВАШ_АККАУНТ" target="_blank" title="VK">
                            <i class="fab fa-vk"></i>
                        </a>
                        <a href="https://wa.me/ВАШ_НОМЕР" target="_blank" title="WhatsApp">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                        <a href="https://t.me/ВАШ_АККАУНТ" target="_blank" title="Telegram">
                            <i class="fab fa-telegram"></i>
                        </a>
                    </div>
                </div>
            </div>
            <div class="foot">
                <span>График работы:</span><br>
                <span>ПН-СБ: 08:00 - 17:00</span><br>
                <span>ВС: выходной</span><br>
                <a href="admin.php">Вход администратора</a>
            </div>
            <div id="desc" class="foot">
                <span>Адрес отдела продаж:</span><br>
                Краснодарский край, г. Белореченск, ул. Железнодорожная, д. 107 <br>
                <span>Адрес склада:</span><br>
                Краснодарский край, Белореченский р-н, п. Новый, ул. Терновая, д. 3
            </div>
        </div>
        <div class="copyright">
            <span>&copy; 2025 Юг-Белора-Металл ИП Ханикян Артур Андроникович</span>
        </div>
    </footer>
    <script src="script/main.js"></script>
    <script>
    document.getElementById('select-table').addEventListener('change', function () {
        const table = this.value;
        const container = document.getElementById('formFields');
        container.innerHTML = ''; // Очистить поля

        if (table === 'products') {
            container.innerHTML = `
                <div class="form-row">
                    <label>Название</label>
                    <input type="text" name="name" required>
                </div>
                <div class="form-row">
                    <label>Цена</label>
                    <input type="number" step="0.01" name="price" required>
                </div>
                <div class="form-row">
                    <label>Категория</label>
                    <select name="category_id" required>
                        <?php
                        $conn = new mysqli("localhost", "root", "", "diplom");
                        $res = $conn->query("SELECT ID, name FROM categories");
                        while ($cat = $res->fetch_assoc()):
                        ?>
                            <option value="<?= $cat['ID'] ?>"><?= htmlspecialchars($cat['name']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="form-row">
                    <label>Еденица товара</label>
                    <input type="text" name="price_for" required>
                </div>
                
                <div class="form-row">
                    <label>Изображение (16:9/4:3)</label>
                    <input type="file" name="img" accept="image/*">
                </div>
                
            `;
        } else if (table === 'categories') {
            container.innerHTML = `
            <div class="form-row">
                <label>Название категории</label>
                <input type="text" name="name" required>
            </div>
                
            `;
        } else if (table === 'attributes') {
            container.innerHTML = `
            <div class="form-row">
                <label>Название атрибута</label>
                <input type="text" name="name" required>
            </div>
                
            `;
        }
        else if (table === 'productattributes') {
            container.innerHTML = `
                <div class="form-row">
                    <label>Товар</label>
                    <select name="product_id" required>
                        <?php
                        $res = $conn->query("SELECT ID, name FROM products");
                        while ($prod = $res->fetch_assoc()):
                        ?>
                            <option value="<?= $prod['ID'] ?>"><?= htmlspecialchars($prod['name']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="form-row">
                    <label>Атрибут</label>
                    <select name="attribute_id" required>
                        <?php
                        $res = $conn->query("SELECT ID, name FROM attributes");
                        while ($attr = $res->fetch_assoc()):
                        ?>
                            <option value="<?= $attr['ID'] ?>"><?= htmlspecialchars($attr['name']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="form-row">
                    <label>Значение</label>
                    <input type="text" name="value" required>
                </div>
                

                
            `;
        }else if (table === 'orderitems') {
        container.innerHTML = `
            <div class="form-row">
                <label>Товар/Услуга</label>
                <select name="product_id" required>
                    <?php
                    $res = $conn->query("SELECT ID, name FROM products");
                    while ($prod = $res->fetch_assoc()):
                    ?>
                        <option value="<?= $prod['ID'] ?>"><?= htmlspecialchars($prod['name']) ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div class="form-row">
                <label>Заказ</label>
                <select name="order_id" required>
                    <?php
                    $res = $conn->query("SELECT ID, phone FROM orders");
                    while ($attr = $res->fetch_assoc()):
                    ?>
                        <option value="<?= $attr['ID'] ?>"><?= htmlspecialchars($attr['ID']) ?> <?= htmlspecialchars($attr['phone'])?></option>
                    <?php endwhile; ?>
                </select>
            </div>
            
            
            
            `;
        }
        
    });
    // Обработка отправки формы
    document.addEventListener("DOMContentLoaded", function () {
        document.getElementById('editForm').addEventListener('submit', function(e) {
        e.preventDefault();
        const formData = new FormData(this);

        fetch('php/update_record.php', {
            method: 'POST',
            body: formData
        }).then(() => {
            document.getElementById('editDialog').classList.remove('active');
            location.reload();
        });
        });
    });
    document.addEventListener("DOMContentLoaded", function () {
    const formFields = document.getElementById("formFields");

    if (!formFields) return;

    // Скрываем блок изначально
    formFields.style.display = "none";

    const observer = new MutationObserver(() => {
        // Проверяем, есть ли значимый контент
        const hasContent = formFields.innerHTML.trim() !== "";

        formFields.style.display = hasContent ? "block" : "none";
    });

    // Наблюдаем за изменениями внутри блока
    observer.observe(formFields, { childList: true, subtree: true });
});
    </script>
</body>
</html>